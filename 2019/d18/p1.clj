...
(defn find-robot [grid]
  (first (filter #(robot? (get-in grid %)) (yx-list grid))))

(defn update-grid-robot-key-door [grid new-pos]
...
(defn update-results [grid cur]
  (if (key? (get-in grid (cur :pos)))
    [{ :cost (+ (count (cur :path)))
       :grid (update-grid-robot-key-door grid (cur :pos))
...
(defn available-moves [grid]
  (let [start (find-robot grid)]
    (loop [queue [{:pos start :path #{}}]
          results []]
      (let [cur (first queue)
            new-results (into results (update-results grid cur))
