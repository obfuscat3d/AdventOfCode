...
(defn make-need [need need-qty rules extra needs]
  (let [unit-qty ((rules need) :qty)
    ...
    [(add-to-map extra need extra-qty) (dissoc (add-new-needs needs units inputs) need)]))

(defn ore-for-fuel [fuel-qty rules]
  (loop [needs {"FUEL" fuel-qty}
         extra {}]
...
(def T 1000000000000N)
(defn bin-search [rules]
  (loop [lo 1N
         hi T]
    (if (>= lo hi)
      (min lo hi)
      (if (< ((ore-for-fuel (quot (+ hi lo) 2) rules) "ORE") T)
        (recur (quot (+ hi lo) 2) hi)
        (recur lo (- (quot (+ hi lo) 2) 1))))))
